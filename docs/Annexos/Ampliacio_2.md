...
* **a) Taula de tipologies d'ccte:** Captura de la nova vista de llista o formulari del model `agrupaciomusical.acte.tipus` on es vegen els diferents imports configurats (colla, extern) i l'import fix per quilòmetre estipulat (ex: $0.30€/\text{km}$).
